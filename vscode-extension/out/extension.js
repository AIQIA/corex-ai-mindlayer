...
function activate(context) {
    ...
    context.subscriptions.push(createAiJsonCommand, validateSchemaCommand, runScannerCommand, architecturePreviewCommand, treeExplorerCommand, aiIntelliSenseCommand, aiJsonWatcher);
}
...
async function showArchitecturePreview() {
    const panel = vscode.window.createWebviewPanel('aiArchitecturePreview', '🏗️ Architecture Preview', vscode.ViewColumn.Beside, {
        enableScripts: true,
...
async function openTreeExplorer() {
    const panel = vscode.window.createWebviewPanel('aiTreeExplorer', '🌳 AI Tree Explorer', vscode.ViewColumn.Beside, {
        enableScripts: true,
        retainContextWhenHidden: true
    });
    const aiJsonContent = await loadAiJsonContent();
...
async function enableAiIntelliSense() {
    ...
    const provider = vscode.languages.registerCompletionItemProvider({ scheme: 'file' }, new AiIntelliSenseProvider(aiJsonContent), '.');
 ...
 */
function getArchitecturePreviewHtml(aiData) {
