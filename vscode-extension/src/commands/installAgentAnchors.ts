...
export async function installAgentAnchors() {
    ...
    const potentialTargets: AnchorTarget[] = [
    ...
    let targetsToInstall: AnchorTarget[] = [];

    if (existingTargets.length > 0) {
        // Falls Dateien existieren, schlage vor, diese zu patchen
        const selected = await vscode.window.showQuickPick(
            existingTargets.map(t => ({ label: t.name, description: t.path, target: t })),
            { canPickMany: true, title: 'Select Agent Anchors to update/install' }
        );
        if (selected) targetsToInstall = selected.map(s => s.target);
    } else {
        // Falls keine existiert, frage welche erstellt werden sollen
        const selected = await vscode.window.showQuickPick(
            potentialTargets.map(t => ({ label: t.name, description: `Create ${t.path}`, target: t })),
            { canPickMany: true, title: 'No Agent Anchors found. Which one should be created?' }
        );
        if (selected) targetsToInstall = selected.map(s => s.target);
    }
