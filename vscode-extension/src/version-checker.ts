...
/**
 * Interface für die Analyse von Schema-Änderungen
 */
interface SchemaChangesInfo {
    // Hinzugefügte Felder
    addedFields: Array<{
        path: string;
        value: any;
    ...
    // Strukturelle Änderungen mit hohem Impakt
    ...
    // Gesamtes Risiko-Level des Updates
    riskLevel: 'low' | 'medium' | 'high';

    // Ob projektspezifische Daten betroffen sind
    projectDataImpact: boolean;

    // Empfehlungen für den Update-Prozess
...
export class VersionChecker {
    ...
    private showUpdateNotification(version: string, changelog: string): void {
        ...
        vscode.window.showInformationMessage(
            updateMessage, 
            'Update sicher installieren', 
            'Details & Änderungen anzeigen', 
            'Später erinnern'
        ).then(selection => {
     ...
     */
    private async startSafeUpdateProcess(version: string, changelog: string): Promise<void> {
     ...
     */
    private showUpdateSafetyDialog(version: string, changelog: string, changes: SchemaChangesInfo, backupPath: string): void {
        // Erstelle ein Webview-Panel für die detaillierte Darstellung
        const panel = vscode.window.createWebviewPanel(
            'aiMindLayerUpdateSafety',
            `AI MindLayer v${version} - Sicherheitsüberprüfung`,
     ...
     */
    private showChangelogWithDiff(version: string, changelog: string): void {
        // Ähnlich wie showUpdateSafetyDialog, aber mit Fokus auf Changelog
        const panel = vscode.window.createWebviewPanel(
            'aiMindLayerChangelog',
            `AI MindLayer v${version} - Änderungsdetails`,
            vscode.ViewColumn.One,
            { enableScripts: true }
        );
     ...
     */
    private showUpdateConfirmationDialog(version: string, changelog: string, changes: SchemaChangesInfo, backupPath: string): void {
        vscode.window.showInformationMessage(
            `Bereit für Update auf v${version}. ${changes.addedFields.length} neue und ${changes.modifiedFields.length} geänderte Felder. Ein Backup wurde erstellt.`,
            { modal: true },
            'Update durchführen',
            'Details anzeigen',
            'Abbrechen'
    ...
    private async installUpdate(version: string): Promise<void> {
        ...
        vscode.window.withProgress({
            ...
            try {
                ...
                vscode.window.showInformationMessage(
                    '🎉 Update erfolgreich und sicher installiert! VS Code muss neu gestartet werden.',
                    'Jetzt neu starten'
                ).then(selection => {
     ...
     */
    private async installVsixSafely(vsixPath: string): Promise<void> {
        ...
        try {
            ...
            const preUpdateSnapshot: Record<string, any> = {};
            ...
            const protectionResults: string[] = [];
            ...
            const updateReportPath = path.join(
                ...
                'update_logs', 
                `update_report_${new Date().toISOString().replace(/[:.]/g, '_')}.json`
            );
     ...
     */
    private mergeAiJsonData(original: any, updated: any): any {
        ...
        for (const section of projectSpecificSections) {
            ...
            let originalValue: any = originalCopy;
            let resultTarget: any = result;
        ...
        if (originalCopy.meta && result.meta) {
            result.meta = {
                ...result.meta,  // Neue Meta-Daten als Basis
     ...
     */
    private safelyMergeJsonData(original: any, updated: any): any {
     ...
     */
    private logSuccessfulUpdate(version: string): void {
        ...
        try {
            ...
            await vscode.window.withProgress({
                ...
                const rollbackReportPath = path.join(
                    this.context.globalStorageUri.fsPath, 
                    'update_logs', 
                    `rollback_report_${new Date().toISOString().replace(/[:.]/g, '_')}.json`
                );
                ...
                if (failedFiles.length === 0) {
                    vscode.window.showInformationMessage(
                        `Rollback erfolgreich! ${restoredFiles.length} Dateien wurden wiederhergestellt.`,
                        'Details anzeigen'
                    ).then(selection => {
                        if (selection === 'Details anzeigen') {
...
`;
                    ...
                    vscode.window.showWarningMessage(
                        `Rollback teilweise erfolgreich. ${restoredFiles.length} Dateien wiederhergestellt, ${failedFiles.length} fehlgeschlagen.`,
                        'Details anzeigen',
                        'Problem melden'
                    ).then(selection => {
                        if (selection === 'Details anzeigen') {
            ...
            vscode.window.showErrorMessage(
                'Automatischer Rollback fehlgeschlagen. Möchten Sie die Backup-Dateien manuell öffnen?',
     ...
     */
    private async analyzeSchemaChanges(currentSchema: any, newSchema: any): Promise<SchemaChangesInfo> {
        // Ergebnisstruktur initialisieren
        const result: SchemaChangesInfo = {
            addedFields: [],
            removedFields: [],
            modifiedFields: [],
            structuralChanges: [],
        ...
        const isProjectData = (path: string): boolean => {
            return criticalProjectDataPaths.some(criticalPath => 
        ...
        const isLikelyUserContent = (path: string): boolean => {
            return userContentPaths.some(contentPath =>
        ...
        const determineImpactLevel = (path: string, changeType: 'addition' | 'removal' | 'modification'): 'low' | 'medium' | 'high' => {
        ...
        const compareObjects = (current: any, updated: any, path: string = '') => {
        ...
        if (result.projectDataImpact || highRiskChanges.length > 0 || result.removedFields.length > 5) {
            ...
            if (highRiskChanges.length > 0) {
                result.recommendations.push(
                    `Es wurden ${highRiskChanges.length} kritische Strukturänderungen gefunden, die Projektdaten betreffen könnten.`
                );
            }
        ...
        result.recommendations.push(
            `Zusammenfassung der Änderungen: ${result.addedFields.length} neue Felder, ` +
     ...
     */
    private generateVisualDiff(changes: SchemaChangesInfo): string {
     ...
     */
    private async createProjectBackup(workspaceFolder: vscode.WorkspaceFolder): Promise<string> {
        try {
            ...
            const criticalFilesToBackup = [
                '.ai.json',                  // Hauptprojektdaten
                '.ai.dev.json',              // Entwicklerspezifische Daten
                '.ai.user.json',             // Benutzerspezifische Daten
                '.ai.research.json',         // Forschungsdaten
                '.ai.preferences.json',      // Benutzereinstellungen
                'schema.json'                // Schema-Definitionen
            ];
            
            // Zusätzliche Dateien, die auch gesichert werden sollten
            const additionalFilesToBackup = [
                '.ai-extensions.json',       // Erweiterungen
                '.ai-components.json',       // Komponentendaten
                'ai-init.php',               // Initialisierungsskript
            ...
            const backupInfo = {
                ...
                safetyChecks: {
                    ...
                    allFilesCopied: successfullyBackedUp.length === allFilesToBackup.filter(file => 
                        fs.existsSync(path.join(workspaceFolder.uri.fsPath, file))).length
                },
                backupLocations: [
                    backupDir,
