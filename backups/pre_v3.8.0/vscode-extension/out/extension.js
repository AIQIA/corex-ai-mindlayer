...
function activate(context) {
    ...
    const updateFromPackageCommand = vscode.commands.registerCommand('aiMindLayer.updateFromPackage', ecosystem_1.updateFromPackageManager);
    const scanDockerConfigCommand = vscode.commands.registerCommand('aiMindLayer.scanDockerConfig', ecosystem_1.scanDockerConfiguration);
    ...
    context.subscriptions.push(createAiJsonCommand, validateSchemaCommand, runScannerCommand, architecturePreviewCommand, treeExplorerCommand, aiIntelliSenseCommand, mindMapCommand, autoDocsCommand, diffAnalyzerCommand, autoSyncCommand, updateFromPackageCommand, scanDockerConfigCommand, userPreferencesCommand, manageResearchCommand, aiJsonWatcher);
    // Register version checker command
    (0, version_checker_1.registerVersionCheckerCommand)(context);
}
...
async function showArchitecturePreview() {
    ...
    const panel = vscode.window.createWebviewPanel('architecturePreview', '🏗️ Architecture Preview', vscode.ViewColumn.Two, { enableScripts: true });
...
async function openTreeExplorer() {
    ...
    const panel = vscode.window.createWebviewPanel('treeExplorer', '🌳 Project Tree Explorer', vscode.ViewColumn.Two, { enableScripts: true });
...
async function enableAiIntelliSense() {
    ...
    const panel = vscode.window.createWebviewPanel('aiIntelliSense', '🧠 AI IntelliSense', vscode.ViewColumn.Two, { enableScripts: true });
...
async function showMindMapVisualizer() {
    ...
    const panel = vscode.window.createWebviewPanel('aiMindMapVisualizer', '🧠 AI Mind Map', vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true });
...
async function showDiffAnalyzer() {
    ...
    const panel = vscode.window.createWebviewPanel('aiDiffAnalyzer', '📊 AI Diff Analyzer', vscode.ViewColumn.Beside, { enableScripts: true });
    panel.webview.html = `<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
 ...
 */
function generateMindMapNodes(aiData) {
    let html = '';
    // Add components
    if (aiData.architecture?.components) {
        aiData.architecture.components.forEach((comp) => {
    ...
    if (aiData.context?.key_concepts) {
        aiData.context.key_concepts.forEach((concept) => {
 ...
 */
async function loadAiJsonContent() {
    const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
    if (!workspaceFolder)
        return null;
    try {
...
class AiIntelliSenseProvider {
    ...
    provideCompletionItems(document, position) {
        ...
        if (this.aiData.architecture?.components) {
            this.aiData.architecture.components.forEach((component) => {
                ...
                item.documentation = new vscode.MarkdownString(`**${component.name}**\n\n${component.description || 'Component from .ai.json'}`);
                completions.push(item);
            });
        }
        // Add completions for key concepts
        if (this.aiData.context?.key_concepts) {
            this.aiData.context.key_concepts.forEach((concept) => {
                const item = new vscode.CompletionItem(concept, vscode.CompletionItemKind.Keyword);
 ...
 */
function getTreeExplorerHtml(aiData) {
 ...
 */
function generateTreeHtml(structure, level = 0) {
 ...
 */
function getArchitecturePreviewHtml(aiJsonContent) {
    ...
    <body>
        ...
        ${components.map((comp) => `
            <div class="component">
                ...
                ${comp.technologies ? `<p><strong>Technologies:</strong> ${comp.technologies.map((tech) => `<span class="tag">${tech}</span>`).join('')}</p>` : ''}
            </div>
        `).join('')}
 ...
 */
function getIntelliSenseHtml(aiJsonContent) {
...
async function runAutoSync() {
    ...
    try {
        ...
        const createOption = await vscode.window.showInformationMessage('Auto-Sync Script nicht gefunden. Möchten Sie das Script erstellen?', 'Script erstellen', 'Abbrechen');
...
`;
                // Schreibe das Script
                await vscode.workspace.fs.writeFile(scriptPath, Buffer.from(scriptContent, 'utf8'));
...
async function editUserPreferences() {
    ...
    try {
        ...
        const createOption = await vscode.window.showInformationMessage('.ai.json nicht gefunden. Möchten Sie zuerst eine .ai.json Datei erstellen?', 'Datei erstellen', 'Abbrechen');
    ...
    try {
        ...
        await vscode.workspace.fs.writeFile(aiJsonPath, Buffer.from(JSON.stringify(aiJson, null, 4), 'utf8'));
...
async function manageResearch() {
    ...
    try {
        ...
        const panel = vscode.window.createWebviewPanel('researchProjects', '🧪 Research & Prototypes', vscode.ViewColumn.One, {
            enableScripts: true,
 ...
 */
function getResearchWebviewContent(webview, aiJson) {
    ...
    const researchItemsHtml = researchItems.map((item, index) => {
 ...
 */
async function addNewResearchItem(aiJsonPath, aiJson) {
    // Optionen für Status
 ...
 */
async function editResearchItem(aiJsonPath, aiJson, index) {
    ...
    research.technologies = technologiesInput ? technologiesInput.split(',').map((t) => t.trim()) : [];
    if (impact) {
 ...
 */
async function removeResearchItem(aiJsonPath, aiJson, index) {
    if (!aiJson.research || index < 0 || index >= aiJson.research.length) {
        return;
    }
    const research = aiJson.research[index];
    // Bestätigung vor dem Löschen
    const confirmation = await vscode.window.showWarningMessage(`Möchten Sie das Forschungsprojekt "${research.name}" wirklich löschen?`, 'Löschen', 'Abbrechen');
 ...
 */
async function updateResearchProgress(aiJsonPath, aiJson, index, progressAction) {
    if (!aiJson.research || index < 0 || index >= aiJson.research.length) {
        return;
    }
    const research = aiJson.research[index];
