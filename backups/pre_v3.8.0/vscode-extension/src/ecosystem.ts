 ...
 */
export async function updateFromPackageManager() {
    ...
    if (hasPackageJson && hasComposerJson && composerInstalled) {
        packageManager = await vscode.window.showQuickPick(
            ['NPM (package.json)', 'Composer (composer.json)'],
            { placeHolder: 'Welchen Package Manager möchten Sie verwenden?' }
        );
        ...
        const fallbackOption = await vscode.window.showQuickPick(
            ['Standalone Composer Plugin', 'PHP Scanner', 'Abbrechen'],
            { 
    ...
    if ((packageManager === 'npm' && !fs.existsSync(npmPluginPath)) || 
        ...
        const copyPlugins = await vscode.window.showQuickPick(
            ['Ja', 'Nein'],
            { placeHolder: `${packageManager.toUpperCase()} Plugin nicht gefunden. Möchten Sie es aus der Extension kopieren?` }
        );
        
        if (copyPlugins !== 'Ja') return;
    ...
    if (packageManager === 'npm') {
        ...
        try {
            ...
            try {
                ...
                const vendorBinPath = path.join(
                    workspaceFolder.uri.fsPath, 
                    'scripts/ecosystem/composer-plugin/vendor/bin'
                );
 ...
 */
export async function scanDockerConfiguration() {
    const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
    if (!workspaceFolder) {
        vscode.window.showErrorMessage('Kein Workspace-Ordner geöffnet');
        return;
    ...
    if (!hasDockerfile && !hasDockerCompose) {
        const createDocs = await vscode.window.showQuickPick(
            ['Ja', 'Nein'],
            { placeHolder: 'Keine Docker-Konfiguration gefunden. Möchten Sie trotzdem Docker-Dokumentation erstellen?' }
        );
        
        if (createDocs !== 'Ja') return;
    }
    ...
    if (!fs.existsSync(dockerScriptPath)) {
        const createScript = await vscode.window.showQuickPick(
            ['Ja', 'Nein'],
            { placeHolder: 'Docker-Scanner-Script nicht gefunden. Möchten Sie es erstellen?' }
        );
        
        if (createScript !== 'Ja') return;
    ...
    const createDocs = await vscode.window.showQuickPick(
        ['Ja', 'Nein'],
        { placeHolder: 'Möchten Sie auch Docker-Dokumentation generieren?' }
    );
 ...
 */
function createNpmPluginFiles(dir: string) {
    // index.js
 ...
 */
function createComposerPluginFiles(dir: string) {
    // composer.json
    const composerJson = {
 ...
 */
function createDockerScripts(dir: string) {
    // docker-scanner.js
 ...
 */
async function runStandalonePhpScanner(workspaceFolder: vscode.WorkspaceFolder): Promise<void> {
    ...
    if (!fs.existsSync(phpScannerPath)) {
        const createScanner = await vscode.window.showQuickPick(
            ['Ja', 'Nein'],
            { placeHolder: 'PHP Scanner nicht gefunden. Möchten Sie ihn erstellen?' }
        );
        
        if (createScanner !== 'Ja') return;
 ...
 */
function createPhpScannerFiles(phpScannerDir: string): void {
 ...
 * @param workspaceFolder Workspace-Ordner
 */
async function runStandaloneComposerPlugin(workspaceFolder: vscode.WorkspaceFolder): Promise<void> {
    // Prüfe, ob der Standalone Composer Plugin existiert
    const standalonePluginPath = path.join(
        workspaceFolder.uri.fsPath, 
        'scripts', 
        'ecosystem', 
        'php-scanner', 
        'StandaloneComposerPlugin.php'
    );
    
    if (!fs.existsSync(standalonePluginPath)) {
        const createPlugin = await vscode.window.showQuickPick(
            ['Ja', 'Nein'],
            { placeHolder: 'Standalone Composer Plugin nicht gefunden. Möchten Sie ihn erstellen?' }
        );
        
        if (createPlugin !== 'Ja') return;
        ...
        if (extensionPath) {
            ...
            if (fs.existsSync(path.join(sourcePath, 'StandaloneComposerPlugin.php'))) {
                fs.copyFileSync(
                    path.join(sourcePath, 'StandaloneComposerPlugin.php'), 
                    standalonePluginPath
                );
            } else {
 ...
 * @param phpScannerDir PHP Scanner Verzeichnis
 */
function createStandaloneComposerPluginFiles(phpScannerDir: string): void {
